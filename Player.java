...
class Player{
    ...
    Player(SHinterface sh, Image cardspic, Graphics g, Hand hand, Image back, Image backSW, Image burntPic, Image[] pointer, Score score)
    {
        this.sh = sh;
    ...
    if(hand.getCard(0) == null){//if player only has cards on table
        if(hand.isFaceUp()){ // if cards still faceup on table
            ...
            if(isValidCard(card, command))
                ...
                sh.addMsg("The card you played was a " + hand.getFaceDown(cardno).getStringValue() + " you had to pick up the pile");
                for(int n = 0; n < 52; n++){
                    if(pile[n] == null)
                        break;
                    hand.addCard(pile[n]);
    ...
    for(int n = 0; n < hand.length() - 1; n++){
        ...
        if(numbertoplay <= 0)
            break;
        if(card.getValue() == hand.getCard(n).getValue() && card.getNumber() != hand.getCard(n).getNumber()){
    ...
    for(int n = 0; n < 3; n++)
        if(hand.getFaceUp(n) != null){
            if(numbertoplay <= 0)
                break;
            if(card.getValue() == hand.getFaceUp(n).getValue() && card.getNumber() != hand.getFaceUp(n).getNumber()){
...
class WaitforMsg implements Runnable{
    ...
    private void out(String otherplayermsg, int commandlength){
        ...
        }catch(NumberFormatException b){ sh.addMsg("Otherplayer - out -  variable to Int error: " + b); }
    ...
    *-------------------------------*/
    private void othersturn(String otherplayermsg, int commandlength)
    {
        ...
        if(cardno.equals("pickup")){//other players picks up pile
            ...
            burn = faceupmulti(otherplayermsg, varlength3, playernumber);
                ...
                }catch(NumberFormatException b){ sh.addMsg("Otherplayer - variable to Int error: " + b); }
            }
            ...
            if(cardno2.equals("pickup")){
                ...
                for(int n = 0; n < 52; n++)
                    pile[n] = null;
                sh.addMsg( othernames[playernumber] + " played a " + Card.getCardStringValue(numPlayed) + " and had to picked up the pile");
            }else{
                try{
                    Card card = new Card(Integer.parseInt(cardno2), cardspic, sh, g);
                    for(int n = 51; n > 0; n--)
            ...
            }catch(NumberFormatException b){ sh.addMsg("Otherplayer else - variable to Int error: " + b); }
            if(deck == 0 || cardcount[playernumber] > 3){
                if(cardcount[playernumber] > 0)
                    cardcount[playernumber]--;
            }else
    ...
    private boolean faceupmulti(String otherplayermsg, int varlength2, int playernumber)
    {
            ...
            try{
                numPlayed = Integer.parseInt(numPlayedString);
            }catch(NumberFormatException b){ sh.addMsg("processTurn - multi face up - variable to Int error: " + b); }
            for(int n = 0; n < numPlayed; n++){
                varlength2 = varlength3;
                //determining how many card where played
                varlength3 = 0;
            ...
            if(hand.isFaceUp())//if player has faceup card on the table
            {
                for(int n = 0; n < 3; n++){
                    if(hand.getFaceUp(n) != null){
                        ...
                        if(canplay)
                            break;
                        if (seven == true && pile[top].getValue() == 7 && hand.getFaceUp(n).getValue() < 7){
                            canplay = true;
                            break;
                        }else if(hand.getFaceUp(n).getValue() == 2 || hand.getFaceUp(n).getValue() == 10){
                            canplay = true;
                            break;
    ...
    private void getcard(String otherplayermsg, int commandlength)
    {
            ...
            try{
                ...
                hand.addCard(card);
            }catch(NumberFormatException b){ sh.addMsg("Deal - variable to Int error: " + b); }
            deck--;
            displayTable();
    }
    ...
    private void otherdetails(String otherplayermsg, int commandlength)
    {
            ...
            for(int n = 0; n < 3; n++)
            {
                ...
                }catch(NumberFormatException b){ sh.addMsg("otherdetails - variable to Int error: " + b); }
            }
        deck = 16;
        if(fastgame)
            deck = 0;
        ...
        if(swap == true && swapdone == false){//if performing card swap
            ...
            sendCommand("swap:" + inhand[0].getNumber() + ":" + inhand[1].getNumber() + ":" + inhand[2].getNumber() + ":" + ontable[0].getNumber() + ":" + ontable[1].getNumber() + ":" + ontable[2].getNumber() + ":");
            swapdone = true;
        }
        }
}
