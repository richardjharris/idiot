...
public class SwapD extends JDialog implements ActionListener {
    ...
    if (pressed.equals(">>")) {
      ...
      if (inhand.length() == 4 && ontable.length() == 4) {
        ...
        JOptionPane.showMessageDialog(
            parent, "You must have 3 in each group.", "Swap Error", JOptionPane.WARNING_MESSAGE);
  ...
  public Card[] getInHand() {
    ...
    for (int n = 0; n < 3; n++) cards[n] = inhand.getCard(n);
    return cards;
  }

  public Card[] getOnTable() {
    Card cards[] = new Card[3];
    for (int n = 0; n < 3; n++) cards[n] = ontable.getCard(n);
    return cards;
  }
}
