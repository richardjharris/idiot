...
public class GameAI {
  ...
  private String playlowest(Hand hand, int lowest, Card card) {
    ...
    if (hand.length() > 1) { // if cards in hand
      ...
      if (lowestcount
          == 0) { // no card other than 10, 2 and 9 (if nine is true) or player must pickup

        if (twocount != 0) return commandcreator("turn:", hand, 2, twocount);
        else if (ninecount != 0) return commandcreator("turn:", hand, 9, ninecount);
        else if (tencount != 0) return commandcreator("turn:", hand, 10, tencount);
        else return ("turn:pickup:");
      ...
      if (lowestcount
        ...
        if (twocount != 0) return commandcreatorFU("turn:faceup:", hand, 2, twocount);
        else if (ninecount != 0) return commandcreatorFU("turn:faceup:", hand, 9, ninecount);
        else if (tencount != 0) return commandcreatorFU("turn:faceup:", hand, 10, tencount);
        else return ("turn:pickup:");
    ...
    } else return facedown(hand, card);
  }
  ...
  private String commandcreator(String command, Hand hand, int value, int number) {
    if (number == 0) return (command.concat("pickup:"));
    if (number > 1) command = command.concat("multi:" + number + ":");
    int addedcount = 0;
    for (int i = 0; i < hand.length() - 1; i++)
      if (hand.getCard(i).getValue() == value) {
        addedcount++;
        command = command.concat(hand.getCard(i).getNumber() + ":");
        if (addedcount == number) break;
      }
    return command;
  }
  ...
  public String swap(Hand hand, boolean nine) {
    ...
    for (int f = 0; f < 6; f++) {
      ...
      for (int n = 0; n < 6; n++) {
        ...
        if (cardvalue == 9 && nine) cardvalue = 16;
        if (cardvalue == 10) cardvalue = 16;
        ...
        if (canbeswapped[n] == null) {
          ...
          canbeswapped[n] = card;
          for (int s = count; s >= n + 1; s--) if (s + 1 < 6) canbeswapped[s + 1] = canbeswapped[s];
    ...
    String command = "swap:";
    for (int n = 0; n < 6; n++) command = command.concat(canbeswapped[n].getNumber() + ":");
